 ...
 *
 * 내가 만들어야 할 때 고려할 부분
 * 1. 어플라이로 접속시 기존의 사용자가 신청한 노선을 보여준다. (페이징 해줘야 함, 이 부분 다시 공부할 것)
 *    그리고 여기서 정렬을 어떻게 해아할지 고민해야함 신청마감이 거의 임박한 순, 최근에 등록된 노선인 순 2개 정도가 가능하지 않을까...
 *    이 부분은 프론트에서 셀렉트 박스로 보여지게 하자.
 * 2. 노선의 검색 기능,
 *    근데 검색어의 정확도를 위해 제한은 있어야 할 것 같은데
 *    (ㅇㅇ구, ㅇㅇ 역으로 제한한다던가 그런 제안을 해 주는 메세지 한줓을 보여주는 게 좋겠다)
